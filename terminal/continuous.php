			...
			<div class="row">
			 <div class="col-12 text-center">
					<div class="defect_key_container" >
						...
						$qry2 = "SELECT * FROM CURDEFECTS INNER JOIN ROLL ON CD_ROLL_ID=ROLL_ID WHERE CD_ROLL_ID='$roll_id' AND CD_TYPE=2 AND (ROUND(CD_ENDLENGTH)=0 OR ROUND(CD_POINT)=0) ORDER BY CD_ID DESC limit 36";
						$res2 = mysqli_query($fit, $qry2);
						if ($res2 && mysqli_num_rows($res2) > 0) {
						?>
							<table class="table borderless_table" style="border: 1px solid #FFFFFF; border-radius:5px;">
								<thead>
									<tr>
								...
								<tbody>
									...
									while ($row2 = mysqli_fetch_assoc($res2)) {
										...
										echo '<tr >';
										echo '<td style="padding:4px 10px;">' . $defectname . '(' . $defect_shortname . ')</td>';
										echo '<td style="padding:4px 10px;">' . $start_length . '</td>';
										echo '<td style="padding:4px 10px;">';
										echo '<a href="continuous_end.php?defect_id=' . $defectid . '&length=' . $length . '&cd_id=' . $cd_id . '">';
										echo '<div class="btn btn-secondary" style="padding:12px 24px; font-weight:bold;">';
										echo 'END';
										echo '</div>';
										echo '</a>';
										echo '</td>';
						...
						<?php
							...
							echo '<div class="text-white text-bold">NO ACTIVE CONTINUOUS DEFECT</div>';
						}
						?>
					</div>
					...
					<div class="vertical_center">
						<div class="defect_key_container">
							<form method="post" autocomplete="off">
								<div class="field">
									<input type="text" class="focus" name="input_text" id="input_text" placeholder="CONTINUOUS" style="width: 90%;" autofocus>
								</div>
							</form>
						</div>
						<div class="defect_key_container">
