...
if (isset($_POST)) {
    ...
    $checkfld= 'USER_LOGINNAME';
    $fields = array();
    $values = array();
    foreach ($_POST as $key => $val) {
        if($key!="CPASS" && $key!="NPASS" ){
    ...
    $set = rtrim($set, ",");
    if (!empty($idvalue)) {
        echo  $qry = "UPDATE " . $tablename . " SET " . $set . " WHERE " . $idname . "='" . $idvalue . "'";
    } else {
        if(isset($checkfld) && !empty($checkfld)){
            ...
            if (mysqli_num_rows($res) == 0) {
                ...
                if($npass == $cpass){
                    $qry = "INSERT INTO " . $tablename . " SET " . $set . " ,USER_PASSWORD='" . $npass ."'";
                }
                else{
                    $_SESSION['ERROR'] = 'Please Re-enter the password correctly.';
                    $error = true;
                }
            }else{
...
header('location: ../user/std_users.php');
?>